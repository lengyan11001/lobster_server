    ...
    function closeMobileMediaPreview() {
      const modal = $("mobileMediaPreviewDialog");
      const body = $("mobileMediaPreviewBody");
      ...
      if (body) {
        ...
        body.querySelectorAll("iframe, img").forEach((media) => media.removeAttribute("src"));
        body.innerHTML = "";
      }
      modal.classList.add("hidden");
      return true;
    ...
    function mediaPreviewKind(url, filename) {
      const value = `${filename || ""} ${url || ""}`.toLowerCase();
      if (/\.(mp4|webm|mov|m4v)(?:[\s?#]|$)/.test(value)) return "video";
      if (/\.(png|jpe?g|webp|gif|bmp|avif)(?:[\s?#]|$)/.test(value)) return "image";
      if (/\.(mp3|wav|m4a|aac|ogg|flac)(?:[\s?#]|$)/.test(value)) return "audio";
      return "document";
    }
    ...
    function startMediaDownload(url, filename) {
      ...
      if (!IS_ANDROID) anchor.target = "_blank";
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      if (IS_ANDROID_APP) toast("已提交到系统下载");
      return true;
    }
    ...
    function recorderProgress(row) {
      const stage = row && row.process_stage;
    ...
    function recorderFormatDuration(value) {
      let seconds = Math.max(0, Number(value || 0));
      if (seconds > 864000) seconds = Math.floor(seconds / 1000);
    ...
    function renderRecorderDeviceState(next = {}) {
      state.recorderDeviceState = { ...state.recorderDeviceState, ...next };
      const value = state.recorderDeviceState;
      const status = $("recorderDeviceStatus");
      if (status) {
        status.textContent = !value.connected
          ? "尚未连接录音设备"
          : value.recording
            ? `${value.paused ? "录音已暂停" : "正在录音"} · ${value.name || "录音设备"}`
            : `已连接 ${value.name || "录音设备"}`;
      }
      const duration = $("recorderDuration");
      if (duration) {
        duration.textContent = recorderFormatDuration(value.duration);
        duration.classList.toggle("hidden", !value.recording && !Number(value.duration || 0));
      }
      const native = recorderNative();
      const supportsControl = !!native && typeof native.startRecorderRecording === "function";
      if ($("recorderConnectBtn")) $("recorderConnectBtn").textContent = value.connected ? "更换设备" : "连接设备";
      if ($("recorderStartBtn")) $("recorderStartBtn").disabled = !value.connected || value.recording || !supportsControl;
      if ($("recorderStopBtn")) $("recorderStopBtn").disabled = !value.connected || !value.recording || !supportsControl;
      if ($("recorderRefreshBtn")) $("recorderRefreshBtn").disabled = !value.connected;
    }

    function readRecorderNativeState() {
      const native = recorderNative();
      if (!native) return null;
      try {
        const value = JSON.parse(native.getRecorderState() || "{}");
        renderRecorderDeviceState(value);
        return value;
      } catch {
        return null;
      }
    ...
    function renderRecorderRecords() {
      ...
      const pager = $("recorderPager");
      if (pager) {
        const pages = Math.max(1, Math.ceil(state.recorderTotal / state.recorderPageSize));
        pager.innerHTML = `<button type="button" class="ghost" data-recorder-page="${state.recorderPage - 1}" ${state.recorderPage <= 1 ? "disabled" : ""}>上一页</button><span>第 ${state.recorderPage} / ${pages} 页</span><button type="button" class="ghost" data-recorder-page="${state.recorderPage + 1}" ${state.recorderPage >= pages ? "disabled" : ""}>下一页</button>`;
      }
    }
    ...
    async function renameRecorderRecord(id, oldName) {
      const displayName = prompt("输入新的录音名称", oldName || "");
    ...
    window.addEventListener("lobster-recorder", (event) => {
      ...
      if (detail.type === "connected") {
        ...
        renderRecorderDeviceState({ connected: true, recording: true, duration: Number(detail.duration || 0) });
        ...
        if (detail.success) {
          state.recorderDeviceFiles = state.recorderDeviceFiles.filter((row) => row.fileName !== detail.fileName);
          renderRecorderDeviceFiles();
        } else {
      ...
      try {
        ...
        const data = await api("/api/h5-chat/mounted-accounts/wechat-auto-reply", {
          ...
          json: {
            ...
            group_invite_welcome_message: String($("mountedWechatGroupInviteWelcomeMessage")?.value || "").trim(),
          },
        });
        state.mountedAccounts = Array.isArray(data.accounts) ? data.accounts : state.mountedAccounts;
        state.mountedAccountDefaults = data.defaults || state.mountedAccountDefaults || {};
    ...
    function renderMediaPreviews(bubble, urls) {
      ...
      urls.forEach((url) => {
        ...
        if (/\.(mp4|webm|mov)(\?|#|$)/.test(low)) {
          ...
          a.type = "button";
          a.className = "media-preview-trigger";
          a.dataset.mediaPreviewUrl = mediaProxyUrl(url, "inline", "lobster-image.png");
          a.dataset.mediaPreviewName = filenameFromUrl(url, "lobster-image.png");
          ...
          el.type = "button";
          el.dataset.mediaPreviewUrl = mediaProxyUrl(url, "inline", filenameFromUrl(url, "lobster-media"));
          el.dataset.mediaPreviewName = filenameFromUrl(url, "lobster-media");
          el.textContent = "打开预览";
        }
        item.appendChild(el);
    ...
    if ($("recorderSubtabs")) $("recorderSubtabs").addEventListener("click", (event) => {
    ...
    if ($("recorderStartBtn")) $("recorderStartBtn").addEventListener("click", () => {
      ...
      native.startRecorderRecording();
    });
    if ($("recorderStopBtn")) $("recorderStopBtn").addEventListener("click", () => {
      const native = recorderNative();
      if (!native || typeof native.stopRecorderRecording !== "function") return toast("请更新到最新版 APK 后使用设备录音控制");
      native.stopRecorderRecording(false);
    });
    ...
    if ($("recorderDeviceFiles")) $("recorderDeviceFiles").addEventListener("click", (event) => {
      ...
      if (deleteButton) {
        ...
        if (!confirm("从录音设备中删除这个文件？删除后不可恢复。")) return;
        native.deleteRecorderFile(deleteButton.dataset.recorderDeviceDelete || "");
        return;
      }
      if (!button) return;
      const native = recorderNative();
    ...
    if ($("recorderServerFiles")) $("recorderServerFiles").addEventListener("click", (event) => {
      ...
      if (deleteButton) {
        deleteRecorderRecord(deleteButton.dataset.recorderServerDelete).catch((err) => toast(err.message || "删除失败"));
        return;
      }
      if (button) showRecorderDetail(button.dataset.recorderDetail).catch((err) => toast(err.message || "详情加载失败"));
    });
    if ($("recorderPager")) $("recorderPager").addEventListener("click", (event) => {
      const button = event.target.closest("[data-recorder-page]");
      if (!button || button.disabled) return;
      state.recorderPage = Math.max(1, Number(button.dataset.recorderPage || 1));
      loadRecorderRecords().catch((err) => toast(err.message || "加载失败"));
    });
    document.querySelectorAll("[data-auth-tab]").forEach((btn) => {
    ...
    $("mobileMediaPreviewBackdrop")?.addEventListener("click", closeMobileMediaPreview);
    $("mobileMediaPreviewCloseBtn")?.addEventListener("click", closeMobileMediaPreview);
    ...
    document.addEventListener("click", (evt) => {
      const preview = evt.target.closest("[data-media-preview-url]");
      if (preview) {
        evt.preventDefault();
        evt.stopPropagation();
    ...
    window.__lobsterHandleBack = function () {
      ...
      const visibleDialogs = Array.from(document.querySelectorAll('[role="dialog"]:not(.hidden)'));
      const dialog = visibleDialogs[visibleDialogs.length - 1];
      if (dialog) {
        const close = dialog.querySelector('button[aria-label*="关闭"], button[id*="Close"], button[id*="Cancel"]');
        if (close) {
          close.click();
          return true;
        }
      }
      if (activeViewKey() !== "office") {
        $("topBackBtn")?.click();
        return true;
      }
      return false;
