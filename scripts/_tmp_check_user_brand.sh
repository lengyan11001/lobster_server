...
db = SessionLocal()
for u in db.query(User).filter(User.username.in_(['test01','test02'])).all():
    print(f'user={u.username} id={u.id} brand_mark={u.brand_mark} role={u.role} balance={u.credits}')
db.close()
"
REMOTE
