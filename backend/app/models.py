...
class IPContentKeyword(Base):
    """Per-user keyword seeds for IP daily content hot-list syncing."""

    __tablename__ = "ip_content_keywords"
    __table_args__ = (
        UniqueConstraint("user_id", "keyword", name="uq_ip_content_keyword_user_keyword"),
        Index("ix_ip_content_keywords_user_status", "user_id", "status"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    user_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    keyword: Mapped[str] = mapped_column(String(191), nullable=False, index=True)
    display_name: Mapped[str] = mapped_column(String(255), default="", nullable=False)
    status: Mapped[str] = mapped_column(String(32), default="active", nullable=False, index=True)
    last_fetch_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    meta: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
...
class IPContentDraftRecord(Base):
    """AI copy records generated by the IP daily content studio."""

    __tablename__ = "ip_content_draft_records"
    __table_args__ = (
        UniqueConstraint("record_id", name="uq_ip_content_draft_records_record_id"),
        Index("ix_ip_content_draft_records_user_created", "user_id", "created_at"),
        Index("ix_ip_content_draft_records_user_task_created", "user_id", "task", "created_at"),
    )
    ...
    platform: Mapped[str] = mapped_column(String(32), default="douyin", nullable=False, index=True)
    title: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    content: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    image_prompt: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    image_url: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    image_asset_id: Mapped[Optional[str]] = mapped_column(String(128), nullable=True, index=True)
...
class H5ChatSession(Base):
    ...
    summary_through_message_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
...
class RecorderAudioRecord(Base):
    ...
    recorded_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
