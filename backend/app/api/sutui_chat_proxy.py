...
_LOBSTER_SYSTEM_HINT = (
    ...
    "**普通视频生成（含图生视频、文生视频）必须用 video.generate，禁止用 comfly.veo 或 comfly.veo.daihuo_pipeline 替代。**"
    "3. 如果调用失败（积分不足、模型错误等），直接将错误信息告知用户，不要尝试用其他方式（搜索、网页等）来替代。"
    "4. 【爆款TVC — 严格条件】**仅当**用户原话中**明确出现**「TVC」「带货视频」「爆款TVC」这些字样时，"
    "才用 capability_id=\"comfly.veo.daihuo_pipeline\"。"
    "**严禁**因为用户文案像广告/口播/带货话术（出现品牌名、slogan、押韵口号、产品介绍等）就主观联想路由到 comfly.veo*——这些场景必须用 video.generate。"
    "**严禁**用户只说 veo3.1/veo/Veo 3.1 Lite 等模型名生成视频时使用 comfly.veo 或 comfly.veo.daihuo_pipeline，必须用 video.generate（payload.model 填模型名如 veo3.1）。"
