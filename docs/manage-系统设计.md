...
## 21. 项目工作安排：人员 + 要求 + 时间节点一起排（可编辑 / 存草稿 / 确认）

老板要的不是"一堆任务名"，而是**一次把三件事排在同一行上**：谁来干、干成什么样、什么时候交。
所以规划的产出直接落成一张「工作安排表」，而不是只给一棵任务树。

### 21.1 交互流程

    AI 生成规划（脑图生长）
      → 自动生成「项目工作安排」表：每行 = 节点 | 人员 | 要求/交付标准 | 时间节点 | 状态
      → AI 按岗位预填责任人与默认要求、按阶段预填起止（可改）
      → 点任意行「编辑」→ 抽屉里改 责任人 / 状态 / 要求 / 开始-结束
      → 【存草稿】随时保存（自动保存 + 手动保存都支持，下次打开可「恢复草稿」续编）
      → 表头实时提示「已排 10/14，还有 4 项待补充」
      → 全部排齐 →【确认安排】→ 抽屉里给确认摘要（人员/要求/时间节点各 N 项 + 最早排期）
      → 确认后节点**仍可编辑**；改动进入「已确认 · 有待确认变更」→ 重新确认生效

### 21.2 校验规则（不满足不许确认）

- 每个节点必须有：责任人、要求、开始与结束时间。
- 缺失项在表里用琥珀色标出（待指派 / 待填写要求 / 待定），表头汇总数量。
- 时间节点必须落在项目周期内；阶段区间不能与子任务矛盾（子任务越界给出警告）。

### 21.3 数据落库

`m_plan_node` 增补字段（其余见 4.4）：

    requirement        TEXT     -- 要求 / 交付标准（老板可改）
    owner_membership_id INTEGER -- 责任人（人）
    owner_ai_employee_id INTEGER-- 责任人（虚拟员工/槽位）
    start_at, end_at   TIMESTAMP-- 时间节点
    status             VARCHAR  -- 未开始 | 进行中 | 完成 | 逾期

`m_plan` 增补状态字段：

    status        VARCHAR  -- draft | confirmed | archived
...
草稿本体（支持多次保存与多版本）：
    ...
    m_arrangement_draft(id, project_id, plan_id, plan_version, payload JSON,
                        saved_by, saved_at)
    -- payload 即整张安排表的快照，恢复草稿时按「节点业务键(类型+标题)」合并，
    -- 保证重规划后同名节点仍能带回老板填过的要求与人员。

### 21.4 状态机

    draft ──确认──▶ confirmed ──编辑──▶ changed ──确认──▶ confirmed
      ▲                                    │
      └────────── 存草稿（任意状态可选）─────┘

- `draft`：可自由编辑，不对外可见（不参与执行）。
- `confirmed`：作为执行基线，可生成下发任务、进入进度跟踪。
- `changed`：已确认版仍生效，改动挂起等确认（避免"改了却没人知道"）。

### 21.5 权限

| 动作 | boss | gm | 被指派人 | 其他人 |
|---|---|---|---|---|
| 编辑任意节点 | 可 | 可 | 仅自己被指派的行（提交进度） | 否 |
| 存草稿 / 恢复草稿 | 可 | 可 | 可（自己的行） | 否 |
| 确认安排 | 可 | 可配 | 否 | 否 |
| 查看安排表 | 本公司全部 | 本公司全部 | 自己相关 | 按项目可见性 |

每次确认写 `m_audit_log`（谁、何时、确认了哪一版、改了什么）。

### 21.6 与执行打通

确认后：指派给**虚拟员工**的节点可「下发」，转成定时任务（见第 8 节），
run 的进度与 `result_payload.targets_detail` 自动回填该行的进度与状态；
指派给**人**的节点出现在其「我的任务」，由本人上报进度与工作记录。
