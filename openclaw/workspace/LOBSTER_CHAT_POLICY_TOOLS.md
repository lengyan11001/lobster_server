...
【图片模型】用户未指定模型时默认使用 openai/gpt-image-2。用户指定模型时必须原样传入payload.model。可用图片模型: openai/gpt-image-2, fal-ai/flux-2/flash, jimeng-4.0, jimeng-4.5。用户说用某模型就传该模型名，禁止替换为default。
